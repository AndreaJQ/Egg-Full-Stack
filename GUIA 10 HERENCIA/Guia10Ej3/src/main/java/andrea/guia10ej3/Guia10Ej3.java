...
public class Guia10Ej3 {
    ...
    public static void main(String[] args) {
        ...
        for (Electrodomestico aux : ListElectr) {
            //Luego, recorrer este array y ejecutar el método precioFinal() en cada electrodoméstico.
                   aux.precioFinal();//llamo al método de la clase padre
                    //si inicio la suma acá sumaría solo el get precio con la info de electro, no incluiría los getprecio de tv(sinto y30%) ni lavadora(extra x carga)
            if (aux instanceof Lavadora) { //el aux de acá es solo para lavadora entoces el getprecio será solo de lavadora.
                ((Lavadora) aux).precioFinalLavadora(); //llamo a método para que sume el precio final incluyendo el precio por carga
               ...
               ((Lavadora) aux).mostrarInfo();
                ...
                ((Televisor) aux).precioFinalTv();//llamo al método de la clase tv para que sume el sintonizador y el 20%
